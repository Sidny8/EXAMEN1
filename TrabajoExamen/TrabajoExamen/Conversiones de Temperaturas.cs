...
namespace TrabajoExamen
{
	...
	public partial class Conversiones_de_Temperaturas : Form
	{
		...
		private bool NumeroA(){
			int num;
			if(!int.TryParse(txtDesde.Text, out num)){
                erpError.SetError(txtDesde,"Debe de poner un numerico");
                txtDesde.Clear();
                txtDesde.Focus();
                return false;
            }
            else{
                erpError.SetError(txtDesde,"");
                return true;
            }
		}
		private bool NumeroB(){
			int num2;
			if(!int.TryParse(txtHasta.Text, out num2)){
                erpError.SetError(txtHasta,"Debe de poner un numerico");
                txtHasta.Clear();
                txtHasta.Focus();
                return false;
            }
            else{
                erpError.SetError(txtHasta,"");
                return true;
            }
		}
		...
		void TxtHastaTextChanged(object sender, EventArgs e)
		{
			if(txtHasta.Text!=""){
				if(NumeroB()==false){
					return;
